...
void main() {
  ...
  // Very dark desaturated burgundy water with muted ash-rose foam
    vec3 dark = vec3(0.08, 0.03, 0.04);       // Почти чёрный бордовый
    vec3 mid  = vec3(0.14, 0.05, 0.07);       // Глубокий винный оттенок
    vec3 foam_color = vec3(0.65, 0.58, 0.57); // Приглушённая пепельно-розовая пена
